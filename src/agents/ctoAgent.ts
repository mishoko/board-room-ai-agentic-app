 ...
 * ```
 */
export async function ctoAgent(state: Record<string, any>, techContext: TechContext): Promise<CTOAssessment> {
...
function analyzeScalability(context: TechContext): { score: number; details: string[] } {
  const { scalabilityNeeds = 'moderate', currentArchitecture = 'monolithic', performanceRequirements = [] } = context;
  ...
  const archScore = architectureScalability[currentArchitecture.toLowerCase()] || 50;
  scalabilityScore = (scalabilityScore + archScore) / 2;
  
  if (archScore < 60) {
    details.push(`Current ${currentArchitecture} architecture limits scalability options`);
  }
  ...
  scalabilityScore *= scalabilityMultiplier[scalabilityNeeds.toLowerCase()] || 1.0;
  
  if (scalabilityNeeds === 'high' || scalabilityNeeds === 'extreme') {
    details.push(`${scalabilityNeeds} scalability requirements may require architecture redesign`);
  }
  
  // Check performance requirements complexity
  const complexPerformanceReqs = performanceRequirements.filter(req =>
  ...
  if (complexPerformanceReqs.length > 0) {
    scalabilityScore -= 15;
    details.push(`Complex performance requirements: ${complexPerformanceReqs.join(', ')}`);
  }
...
function analyzeTechDebt(context: TechContext): { score: number; details: string[] } {
  ...
  if (techDebtLevel > 70) {
    debtScore -= 20;
    details.push(`High technical debt (${techDebtLevel}%) will slow development velocity`);
  }
  
  // Legacy architecture adds debt risk
  if (currentArchitecture.toLowerCase() === 'legacy') {
    debtScore -= 25;
    details.push('Legacy architecture increases technical debt accumulation risk');
  }
  
  // Moderate debt requires careful planning
  if (techDebtLevel > 40 && techDebtLevel <= 70) {
    details.push(`Moderate technical debt requires careful feature planning to avoid accumulation`);
  }
...
function analyzeResourceNeeds(context: TechContext): { score: number; details: string[] } {
  ...
  const complexSkills = requiredSkills.filter(skill =>
    ['Kubernetes', 'Machine Learning', 'Blockchain', 'WebAssembly', 'Rust', 'Go'].some(complex =>
  ...
  if (complexSkills.length > 0) {
    ...
    details.push(`Complex skills required: ${complexSkills.join(', ')} - may need specialized hiring`);
  }
  
  // Check resource availability
  if (resourceAvailability < 60) {
    details.push(`Limited engineering capacity (${resourceAvailability}%) may constrain development`);
  }
  
  // Assess skill diversity requirements
  if (requiredSkills.length > 5) {
    resourceScore -= 10;
    details.push(`High skill diversity (${requiredSkills.length} skills) increases coordination complexity`);
  }
...
function analyzeArchitectureFit(context: TechContext): { score: number; details: string[] } {
  ...
  const modernSkills = requiredSkills.filter(skill =>
    ['React', 'Node.js', 'Docker', 'Kubernetes', 'GraphQL', 'TypeScript'].some(modern =>
      skill.toLowerCase().includes(modern.toLowerCase())
    )
  );
  ...
  if (modernSkills.length > 0 && currentArchitecture.toLowerCase() === 'legacy') {
    fitScore -= 30;
    details.push('Modern technology requirements conflict with legacy architecture');
  }
  ...
  if (microservicesSkills.length > 0 && currentArchitecture.toLowerCase() === 'monolithic') {
    fitScore -= 20;
    details.push('Microservices technologies may require architectural migration');
  }
...
function analyzeSecurityRequirements(context: TechContext): { score: number; details: string[] } {
  ...
  const highSecurityReqs = securityRequirements.filter(req =>
  ...
  if (highSecurityReqs.length > 0) {
    ...
    details.push(`High security requirements: ${highSecurityReqs.join(', ')} - requires specialized expertise`);
  }
  ...
  if (complianceReqs.length > 0) {
    securityScore -= 20;
    details.push(`Regulatory compliance required: ${complianceReqs.join(', ')} - adds development overhead`);
  }
...
async function generateCTOOutput(proposal: string, context: TechContext, analyses: any): Promise<CTOAssessment> {
  const template = ChatPromptTemplate.fromMessages([
    ['system', `You are an experienced CTO analyzing a technical proposal from an engineering and architecture perspective.
...
Return JSON response with assessment (approve/reject/neutral), confidence (0-100), reasoning, techConcerns array, and recommendations array:`]
  ...
  return callLLM({
    prompt: { toMessages: () => template.formatMessages({
      ...
      performanceRequirements: context.performanceRequirements?.join(', ') || 'Not specified',
      securityRequirements: context.securityRequirements?.join(', ') || 'Not specified',
      ...
      techDebtDetails: analyses.techDebt.details.join('; '),
      resourceScore: analyses.resources.score,
      resourceDetails: analyses.resources.details.join('; '),
      architectureScore: analyses.architecture.score,
      architectureDetails: analyses.architecture.details.join('; '),
      securityScore: analyses.security.score,
      securityDetails: analyses.security.details.join('; ')
    }) },
    model: CTOAssessment,
    agentName: 'ctoAgent',
    defaultFactory: () => {
      const assessment = new CTOAssessment();
      assessment.reasoning = 'Error in generating technical analysis - defaulting to neutral assessment';
      assessment.confidence = 50;
      assessment.techConcerns = ['Unable to complete full technical analysis due to system error'];
      assessment.recommendations = ['Retry analysis with updated technical context'];
      return assessment;
    }
  });
}
