...
export class SummaryAgent {
  ...
  public generateConversationSummary(
    ...
    const summary = this.generateDetailedSummary(topic, messages, participants.size);
  ...
  private generateDetailedSummary(
    ...
    let summary = `The board conducted a${intensity === 'intense' ? 'n intense' : intensity === 'brief' ? ' brief' : ''} discussion on ${topic.title.toLowerCase()}`;
    
    if (userMessages.length > 0) {
      summary += ` with stakeholder input`;
    }
    
    summary += `. ${executiveRoles.length} executives participated, bringing diverse perspectives from ${executiveRoles.join(', ').toLowerCase()} functions.`;
    ...
    summary += ` Given ${this.companyContext.name}'s position as a ${this.companyContext.stage} ${this.companyContext.industry} company, the discussion focused on ${this.getContextualFocus(topic, messages)}.`;
  ...
  private getExecutiveRoles(messages: Message[]): string[] {
    ...
    messages.forEach(msg => {
      if (msg.agentId !== 'user') {
        ...
        if (msg.agentId.includes('CEO') || msg.agentId.includes('ceo')) roles.add('CEO');
        if (msg.agentId.includes('CTO') || msg.agentId.includes('cto')) roles.add('CTO');
        if (msg.agentId.includes('CFO') || msg.agentId.includes('cfo')) roles.add('CFO');
        if (msg.agentId.includes('CMO') || msg.agentId.includes('cmo')) roles.add('CMO');
        if (msg.agentId.includes('CHRO') || msg.agentId.includes('chro')) roles.add('CHRO');
        if (msg.agentId.includes('COO') || msg.agentId.includes('coo')) roles.add('COO');
      }
    });
  ...
  private analyzeExecutivePerspectives(messages: Message[]): {
    ...
    const agentMessages = messages.filter(msg => msg.agentId !== 'user');
    const allText = agentMessages.map(msg => msg.text).join(' ').toLowerCase();
    
    // Look for consensus indicators
    const consensusWords = ['agree', 'aligned', 'consensus', 'support', 'endorse'];
    const disagreementWords = ['disagree', 'concerned', 'challenge', 'question', 'oppose', 'risk'];
    const challengingWords = ['however', 'but', 'although', 'questioning', 'challenging'];
    
    const consensusCount = consensusWords.filter(word => allText.includes(word)).length;
    const disagreementCount = disagreementWords.filter(word => allText.includes(word)).length;
    const challengingCount = challengingWords.filter(word => allText.includes(word)).length;
    
    const consensus = consensusCount > disagreementCount && challengingCount < 3;
    const majorDisagreement = disagreementCount > consensusCount * 1.5 || challengingCount > 5;
  ...
  private getContextualFocus(topic: Topic, messages: Message[]): string {
    ...
    } else if (allText.includes('financial') || allText.includes('cost') || allText.includes('budget')) {
      return `financial implications and capital allocation priorities for a ${stage} company`;
    } else if (allText.includes('technical') || allText.includes('technology')) {
  ...
  private determineOutcome(allText: string, topic: Topic): string | null {
    // Analyze conversation tone and content to determine outcome
    const positiveWords = ['approved', 'proceed', 'move forward', 'implement', 'execute', 'green light'];
    const negativeWords = ['rejected', 'postpone', 'table', 'reconsider', 'not ready', 'too risky'];
    const neutralWords = ['further analysis', 'more research', 'additional information', 'next phase'];
    
    const positiveCount = positiveWords.filter(word => allText.includes(word)).length;
    const negativeCount = negativeWords.filter(word => allText.includes(word)).length;
    const neutralCount = neutralWords.filter(word => allText.includes(word)).length;
    ...
    } else if (neutralCount > 0 || (positiveCount === negativeCount && positiveCount > 0)) {
  ...
  private extractKeyDecisions(messages: Message[]): string[] {
    ...
    agentMessages.forEach(msg => {
      ...
      if (text.includes('we need to') || text.includes('we should') || text.includes('we must')) {
        // Extract the decision/action
        const sentences = msg.text.split(/[.!?]+/);
        sentences.forEach(sentence => {
          if (sentence.toLowerCase().includes('we need to') || 
    ...
    if (allText.includes('technical') && allText.includes('architecture')) {
      decisions.push('Review technical architecture and infrastructure requirements');
    }
    if (allText.includes('market') && allText.includes('research')) {
      decisions.push('Perform additional market research and competitive analysis');
    }
  ...
  private extractMainConcerns(messages: Message[]): string[] {
    ...
    agentMessages.forEach(msg => {
      const text = msg.text.toLowerCase();
      if (text.includes('concern') || text.includes('risk') || text.includes('challenge') || text.includes('problem')) {
        // Extract the concern
        const sentences = msg.text.split(/[.!?]+/);
        sentences.forEach(sentence => {
          if (sentence.toLowerCase().includes('concern') || 
  ...
  private extractNextSteps(messages: Message[], topic: Topic): string[] {
    ...
    agentMessages.forEach(msg => {
      const text = msg.text.toLowerCase();
      if (text.includes('next step') || text.includes('follow up') || text.includes('action item')) {
        const sentences = msg.text.split(/[.!?]+/);
        sentences.forEach(sentence => {
    ...
    // Generate contextual next steps based on conversation content
    const allText = messages.map(msg => msg.text).join(' ').toLowerCase();
    
    if (allText.includes('analysis') || allText.includes('research')) {
      nextSteps.push(`Conduct comprehensive analysis of ${topic.title.toLowerCase()} requirements and implications`);
    }
    if (allText.includes('stakeholder') || allText.includes('team')) {
      nextSteps.push('Engage relevant stakeholders and gather additional input');
    }
    if (allText.includes('timeline') || allText.includes('schedule')) {
      nextSteps.push('Develop detailed implementation timeline and milestones');
    }
    if (allText.includes('budget') || allText.includes('financial')) {
      nextSteps.push('Prepare detailed financial projections and budget requirements');
    }
    
    // Always include a follow-up meeting
    nextSteps.push(`Schedule follow-up board meeting to review progress on ${topic.title.toLowerCase()}`);
