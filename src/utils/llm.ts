...
async function detectLLMConfig(
  url: string
...
export async function callLLM<T>({
  ...
  try {
    if (actualProvider === "openai") {
      ...
      // Force using llama3.2:latest model
      config.model = import.meta.env.VITE_OLLAMA_MODEL || "llama3.2:latest"

      console.log(
        `🔍 Using LLM config for ${agentName}: ${config.model} via ${config.endpoint}`
      )
      ...
      if (response.status === 200 && response.data) {
        ...
        if (responseText) {
          try {
            ...
            console.warn(
              `JSON parse failed for ${agentName}, trying fallback...`
            )
